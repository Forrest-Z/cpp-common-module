...
enum {
	START, RUNNING, END
} TaskState = START;/* enum {START, RUNNING, END} */
...
u8 CloudControlAddTask(u8 *arr, u8 len) {
	...
	/* 无存储空间 */
	if (!(queue.num < QUEUEMAXNUM))//当队列号（任务数）≥ 20，无法增加任务。
		return 0;

	/* 存数据 */
	{
		u8 datalen;
		command[queue.rear].task = task;	//增加CXXX
		datalen = len - 2;               	//不需要CXXX
		command[queue.rear].len = datalen;//该指令的数据长度

		for (u8 i = 0; i < datalen; i++)
			command[queue.rear].data[i] = arr[i + 2];
	}

	/* num加1,rear指向下一位 */
	queue.num++;
	queue.rear++;
	if (queue.rear >= QUEUEMAXNUM)
		queue.rear = 0;
...
static void TaskMove(void) {
	...
	if (TaskState == END) {
		...
		TaskState = START;
		//==============================================================================================
		LogicOutput.hostComputerCommand = COMMAND_NONE;  //消除停车之后行走指令的返回信号不一致
		//==============================================================================================
		return;
	}
	...
	/* 获取避障减速 */
	{
      ...
      r = 1;
		...
		else if((r == CarParameter.OBAV_LimitLevel1) || ( r == CarParameter.OBAV_LimitLevel2 ))
		{
	...
	if (dir == FORWARD || dir == FORWARD_R || dir == FORWARD_L /* */
	|| dir == BACKWARD || dir == BACKWARD_L || dir == BACKWARD_R) {
		...
		LogicOutput.move.omega = 0;
	/*横移用一条磁条*/
		LogicOutput.move.E_F = (CarNavi.delta_F ) / 2;//能加后舵轮偏差吗？+ CarNavi.delta_B
...
void CloudControlProcess(void) {
	...
	if (CarInfo.green_button == 1)
		LogicOutput.move.OBAV_area = 0x07;
